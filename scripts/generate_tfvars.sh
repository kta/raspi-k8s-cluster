...
if [[ "$ENV" != "production" && "$ENV" != "vagrant" ]]; then
	echo "❌ Error: Environment must be 'production' or 'vagrant'"
...
CLUSTER_ENV=$(extract_var "cluster_env")
METALLB_IP_RANGE=$(extract_var "metallb_ip_range")
INGRESS_IP=$(extract_var "ingress_ip")
VIP=$(extract_var "vip")

# Validate extracted variables
if [[ -z "$CLUSTER_ENV" || -z "$METALLB_IP_RANGE" || -z "$INGRESS_IP" || -z "$VIP" ]]; then
	echo "❌ Error: Failed to extract required variables from inventory"
	echo "Required variables: cluster_env, metallb_ip_range, ingress_ip, vip"
	exit 1
fi
...
echo "✅ Generated Terraform variables: $OUTPUT_FILE"
echo ""
echo "📝 Contents:"
cat "$OUTPUT_FILE"
echo ""
