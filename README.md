# Proxmox HA Kubernetes Cluster (GitOps Edition)

Raspberry Pi 5 × 3台で構築する高可用性 Kubernetes クラスター

## 🎯 プロジェクト概要

このプロジェクトは、Terraform と Cloud-Init を使用して、Proxmox VE 上に完全自動化された Kubernetes クラスターを構築します。

### 主要コンポーネント

- **物理層**: Raspberry Pi 5 (8GB) × 3台
- **仮想化**: Proxmox VE (Pimox)
- **ストレージ**: Ceph 分散ストレージ (3ノード構成)
- **Kubernetes**: v1.35 (kubeadm)
- **CNI**: Flannel または Cilium
- **GitOps**: ArgoCD
- **IaC**: Terraform + Cloud-Init

## 📐 アーキテクチャ

```
┌─────────────────┬─────────────────┬─────────────────┐
│   pi-node-1     │   pi-node-2     │   pi-node-3     │
│  (.101)         │  (.102)         │  (.103)         │
├─────────────────┼─────────────────┼─────────────────┤
│ pve-vm-cp-1     │ pve-vm-cp-2     │ pve-vm-cp-3     │
│ (.201)          │ (.202)          │ (.203)          │
│                 │                 │                 │
│ pve-vm-wk-1     │ pve-vm-wk-2     │ pve-vm-wk-3     │
│ (.211)          │ (.212)          │ (.213)          │
└─────────────────┴─────────────────┴─────────────────┘
              ↑
         kube-vip VIP: 192.168.100.200
```

## 🚀 セットアップ手順

### 前提条件（インフラ）

1. ✅ Raspberry Pi 5 × 3台に Proxmox VE (Pimox) がインストール済み
2. ✅ Proxmox クラスターが構成済み (3ノード)
3. ✅ Ceph ストレージプールが構築済み (プール名: `ceph-vm`)
4. ✅ 有線 LAN ネットワーク設定完了
5. ✅ Proxmox API トークンが発行済み

### 開発環境セットアップ（ローカルマシン）

Homebrewで必要なツールをインストール：

```bash
# Terraform バージョン管理ツール
brew install tfenv

# Terraform を指定バージョンでインストール
tfenv install $(cat .terraform-version)
tfenv use $(cat .terraform-version)

# Terraform のリンティングツール
brew install tflint

# TFLint プラグインの初期化
tflint --init

# Kubernetes CLI
brew install kubectl

# （オプション）Proxmox 管理用
brew install jq curl
```

インストール確認：

```bash
terraform version   # 1.10.5 が表示されるはず
tflint --version
kubectl version --client
```

## 🎯 開発タスク

進捗は [TODO.md](./TODO.md) で管理しています。

## 📚 ドキュメント

- [Copilot Instructions](./.github/copilot-instructions.md)

## 🔄 CI/CD

### GitHub Actions
このプロジェクトは以下の自動チェックを実行します：

- **Terraform CI** - コードフォーマット、Validate、Lint、セキュリティスキャン
- **PR Auto Labeler** - 変更ファイルに基づいた自動ラベリング
- **Release** - タグ作成時の自動リリース生成

詳細は [GitHub Actions ドキュメント](./.github/workflows/README.md) を参照してください。

---

**Built with ❤️ for Raspberry Pi Kubernetes**
