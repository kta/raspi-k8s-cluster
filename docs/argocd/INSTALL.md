# ArgoCD 手動インストール手順書

```
  ___                   ____ ____      _____          _        _ _
 / _ \                 / ___|  _ \    |_   _|        | |      | | |
/ /_\ \_ __ __ _  ___ | |   | | | |     | | _ __  ___| |_ __ _| | |
|  _  | '__/ _` |/ _ \| |   | | | |     | || '_ \/ __| __/ _` | | |
| | | | | | (_| | (_) | |___| |_| |    _| || | | \__ \ || (_| | | |
\_| |_/_|  \__, |\___/ \____|____/     \___/_| |_|___/\__\__,_|_|_|
            __/ |
           |___/     Raspberry Pi Kubernetes Cluster Edition
```

---

## 目次

1. [はじめに](#1-はじめに)
2. [前提条件](#2-前提条件)
3. [インストール手順](#3-インストール手順)
4. [初期設定](#4-初期設定)
5. [動作確認](#5-動作確認)
6. [基本的な使い方](#6-基本的な使い方)
7. [トラブルシューティング](#7-トラブルシューティング)
8. [アンインストール](#8-アンインストール)

---

## 1. はじめに

### ArgoCDとは？

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│    ┌──────────────┐         ┌──────────────┐         ┌──────────────┐       │
│    │              │         │              │         │              │       │
│    │  Git Repo    │────────▶│   ArgoCD     │────────▶│  Kubernetes  │       │
│    │  (宣言的設定)  │         │  (同期監視)   │         │  (クラスタ)   │       │
│    │              │         │              │         │              │       │
│    └──────────────┘         └──────────────┘         └──────────────┘       │
│                                    │                                        │
│                                    ▼                                        │
│                          ┌──────────────────┐                               │
│                          │  自動デプロイ      │                               │
│                          │  差分検知         │                               │
│                          │  ロールバック      │                               │
│                          └──────────────────┘                               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

ArgoCDは**GitOps**を実現するためのツールです：
- Gitリポジトリの設定を**信頼できる唯一の情報源**として使用
- クラスタの状態をGitと**自動的に同期**
- 変更の**履歴管理**と**ロールバック**が容易

---

## 2. 前提条件

### 必要なもの

| 項目 | 要件 | 確認コマンド |
|------|------|--------------|
| Kubernetesクラスタ | 稼働中であること | `kubectl get nodes` |
| kubectl | インストール済み | `kubectl version --client` |
| Helm | v3.x以上 | `helm version` |
| kubeconfig | 設定済み | `kubectl cluster-info` |

### クラスタ状態の確認

```bash
# ノードが Ready 状態か確認
kubectl get nodes

# 期待される出力:
# NAME     STATUS   ROLES           AGE   VERSION
# node-1   Ready    control-plane   Xd    v1.3X.X
# node-2   Ready    control-plane   Xd    v1.3X.X
# node-3   Ready    control-plane   Xd    v1.3X.X
```

```bash
# クラスタ情報を確認
kubectl cluster-info

# 期待される出力:
# Kubernetes control plane is running at https://X.X.X.X:8443
# CoreDNS is running at ...
```

### Helmのインストール（未インストールの場合）

```bash
# macOS
brew install helm

# Linux (curl)
curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | bash

# インストール確認
helm version
```

---

## 3. インストール手順

### Step 1: Namespaceの作成

```bash
kubectl create namespace argocd
```

```
✅ 確認ポイント
$ kubectl get namespace argocd
NAME     STATUS   AGE
argocd   Active   Xs
```

### Step 2: Helmリポジトリの追加

```bash
# ArgoCDのHelmリポジトリを追加
helm repo add argo https://argoproj.github.io/argo-helm

# リポジトリ情報を更新
helm repo update
```

```
✅ 確認ポイント
$ helm search repo argo/argo-cd
NAME            CHART VERSION   APP VERSION   DESCRIPTION
argo/argo-cd    X.X.X           vX.X.X        A Helm chart for Argo CD...
```

### Step 3: values.yamlの確認と編集

```bash
# このリポジトリに同梱のvalues.yamlを使用
# 必要に応じて編集してください

# 現在のディレクトリから確認
cat docs/argocd/values.yaml
```

**主要な設定項目:**

| 設定 | デフォルト値 | 説明 |
|------|-------------|------|
| `server.service.type` | NodePort | サービス公開方法 |
| `server.service.nodePortHttps` | 30443 | HTTPSポート番号 |
| `server.extraArgs` | --insecure | HTTPS無効化（開発用） |
| `redis-ha.enabled` | false | 高可用性Redis（無効） |
| `dex.enabled` | false | 外部認証（無効） |

### Step 4: ArgoCDのインストール

```bash
# このリポジトリのルートディレクトリで実行
helm install argocd argo/argo-cd \
  --namespace argocd \
  --values docs/argocd/values.yaml \
  --wait
```

```
┌─────────────────────────────────────────────────────────────────────┐
│ 💡 ヒント                                                           │
├─────────────────────────────────────────────────────────────────────┤
│ --wait オプションにより、すべてのPodがReadyになるまで待機します。    │
│ Raspberry Pi環境では数分かかることがあります。                       │
└─────────────────────────────────────────────────────────────────────┘
```

### Step 5: インストール確認

```bash
# Podの状態確認
kubectl get pods -n argocd

# 期待される出力（すべてRunning/1/1）:
# NAME                                               READY   STATUS    RESTARTS   AGE
# argocd-application-controller-0                   1/1     Running   0          Xm
# argocd-applicationset-controller-XXXXXXXXX-XXXXX  1/1     Running   0          Xm
# argocd-notifications-controller-XXXXXXXXX-XXXXX   1/1     Running   0          Xm
# argocd-redis-XXXXXXXXX-XXXXX                      1/1     Running   0          Xm
# argocd-repo-server-XXXXXXXXX-XXXXX                1/1     Running   0          Xm
# argocd-server-XXXXXXXXX-XXXXX                     1/1     Running   0          Xm
```

```bash
# Serviceの確認
kubectl get svc -n argocd

# argocd-server の NodePort を確認
# argocd-server   NodePort   X.X.X.X   <none>   80:30080/TCP,443:30443/TCP
```

---

## 4. 初期設定

### 管理者パスワードの取得

```bash
# 初期パスワードを取得（自動生成されたもの）
kubectl -n argocd get secret argocd-initial-admin-secret \
  -o jsonpath="{.data.password}" | base64 -d; echo
```

```
┌─────────────────────────────────────────────────────────────────────┐
│ 🔐 セキュリティ注意                                                  │
├─────────────────────────────────────────────────────────────────────┤
│ 初期パスワードは必ず変更してください！                               │
│ 後述の「パスワード変更」セクションを参照してください。               │
└─────────────────────────────────────────────────────────────────────┘
```

### ArgoCD CLI のインストール（オプション）

```bash
# macOS
brew install argocd

# Linux
curl -sSL -o argocd-linux-amd64 \
  https://github.com/argoproj/argo-cd/releases/latest/download/argocd-linux-amd64
sudo install -m 555 argocd-linux-amd64 /usr/local/bin/argocd
rm argocd-linux-amd64

# ARM64 Linux (Raspberry Pi)
curl -sSL -o argocd-linux-arm64 \
  https://github.com/argoproj/argo-cd/releases/latest/download/argocd-linux-arm64
sudo install -m 555 argocd-linux-arm64 /usr/local/bin/argocd
rm argocd-linux-arm64
```

### CLIでログイン

```bash
# port-forwardしてアクセスできるようにする
kubectl port-forward svc/argocd-server -n argocd 8080:443

# ノードのIPアドレスを使用（例: 192.168.1.100, Vagrant環境は 192.168.56.100）
# argocd login <NODE_IP>:30443 --insecure
argocd login localhost:8080 --insecure

# Username: admin
# Password: （上で取得した初期パスワード）
```

### パスワード変更

```bash
# CLIで変更
argocd account update-passwordargocd account update-password

# または Web UI から変更
# Settings → User Info → UPDATE PASSWORD
```

---

## 5. 動作確認

### Web UIへのアクセス

```
┌─────────────────────────────────────────────────────────────────────┐
│                                                                     │
│   ブラウザで以下のURLにアクセス:                                        │
│                                                                     │
│   🌐  https://<NODE_IP>:30443                                       │
│                                                                     │
│   例: https://192.168.1.100:30443                                   │
│       https://raspi-node1:30443                                     │
│       https://localhost:8080                                       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘

ログイン情報:
  - Username: admin
  - Password: （取得した初期パスワード）
```

### ポートフォワードでのアクセス（代替方法）

```bash
# ローカルの8080ポートでアクセス可能にする
kubectl port-forward svc/argocd-server -n argocd 8080:443

# ブラウザで https://localhost:8080 にアクセス
```

### 接続テスト

```bash
# ArgoCD サーバーのバージョン確認
argocd version

# クラスタ一覧（デフォルトでin-clusterが登録済み）
argocd cluster list
```

---

## 6. 基本的な使い方

### サンプルアプリケーションのデプロイ

```bash
# サンプルアプリをデプロイ（ArgoCDの公式サンプル）
argocd app create guestbook \
  --repo https://github.com/argoproj/argocd-example-apps.git \
  --path guestbook \
  --dest-server https://kubernetes.default.svc \
  --dest-namespace default

# アプリケーションを同期（デプロイ）
argocd app sync guestbook
```

```
アプリケーションの状態確認:

┌──────────────────────────────────────────────────────────────┐
│  $ argocd app get guestbook                                  │
│                                                              │
│  Name:               guestbook                               │
│  Project:            default                                 │
│  Server:             https://kubernetes.default.svc          │
│  Namespace:          default                                 │
│  Health Status:      Healthy  ✅                             │
│  Sync Status:        Synced   ✅                             │
└──────────────────────────────────────────────────────────────┘
```

### Web UIでの操作

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          ArgoCD Web UI                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────┐                                                        │
│  │ + NEW APP       │  ← 新しいアプリケーションを作成                         │
│  └─────────────────┘                                                        │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ guestbook                                              ✅ Healthy   │    │
│  │ ─────────────────────────────────────────────────────────────────── │    │
│  │ 🔄 SYNC   📊 DETAILS   🗑️  DELETE                                   │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                             │
│  アプリをクリックすると、リソースの依存関係がツリー表示されます             │
│                                                                             │
│         ┌───────────┐                                                       │
│         │   App     │                                                       │
│         └─────┬─────┘                                                       │
│               │                                                             │
│       ┌───────┴───────┐                                                     │
│       ▼               ▼                                                     │
│  ┌─────────┐    ┌─────────────┐                                             │
│  │ Service │    │ Deployment  │                                             │
│  └─────────┘    └──────┬──────┘                                             │
│                        │                                                    │
│                        ▼                                                    │
│                  ┌──────────┐                                               │
│                  │ ReplicaSet│                                              │
│                  └─────┬────┘                                               │
│                        │                                                    │
│                        ▼                                                    │
│                   ┌────────┐                                                │
│                   │  Pod   │                                                │
│                   └────────┘                                                │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Applicationマニフェストの例

自分のアプリケーションをGitOpsで管理する場合:

```yaml
# app.yaml
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: my-app
  namespace: argocd
spec:
  project: default
  source:
    repoURL: https://github.com/your-username/your-repo.git
    targetRevision: HEAD
    path: kubernetes/manifests
  destination:
    server: https://kubernetes.default.svc
    namespace: my-app-namespace
  syncPolicy:
    automated:
      prune: true      # Git から削除されたリソースを自動削除
      selfHeal: true   # 手動変更を自動で元に戻す
    syncOptions:
      - CreateNamespace=true  # Namespaceを自動作成
```

```bash
# 適用
kubectl apply -f app.yaml
```

---

## 7. トラブルシューティング

### よくある問題と解決方法

#### ❌ Podが起動しない / CrashLoopBackOff

```bash
# Podの詳細確認
kubectl describe pod -n argocd <POD_NAME>

# ログ確認
kubectl logs -n argocd <POD_NAME>
```

**考えられる原因:**
- メモリ不足 → values.yaml のリソース制限を調整
- イメージのPullエラー → ネットワーク接続を確認

#### ❌ Web UIにアクセスできない

```bash
# Serviceの確認
kubectl get svc -n argocd argocd-server

# NodePortが開いているか確認
kubectl get svc argocd-server -n argocd -o jsonpath='{.spec.ports[*].nodePort}'
```

**チェックリスト:**
- [ ] 正しいノードIPを使用しているか
- [ ] ポート番号は正しいか（デフォルト: 30443）
- [ ] ファイアウォールでポートがブロックされていないか

#### ❌ 初期パスワードが取得できない

```bash
# Secretの存在確認
kubectl get secret -n argocd argocd-initial-admin-secret

# 存在しない場合、パスワードをリセット
kubectl -n argocd patch secret argocd-secret \
  -p '{"stringData": {"admin.password": "'$(htpasswd -bnBC 10 "" newpassword | tr -d ':\n')'"}}'
```

#### ❌ アプリケーションの同期が失敗する

```bash
# アプリケーションの詳細確認
argocd app get <APP_NAME>

# 同期ステータス確認
argocd app sync <APP_NAME> --dry-run
```

**よくある原因:**
- Gitリポジトリへのアクセス権限がない
- マニフェストの文法エラー
- リソースの依存関係の問題

### ログの確認方法

```bash
# 全コンポーネントのログを確認
kubectl logs -n argocd -l app.kubernetes.io/part-of=argocd --all-containers

# 特定コンポーネントのログ
kubectl logs -n argocd deployment/argocd-server
kubectl logs -n argocd deployment/argocd-repo-server
kubectl logs -n argocd statefulset/argocd-application-controller
```

### リソース使用状況の確認

```bash
# Podのリソース使用状況
kubectl top pods -n argocd

# メモリ不足の場合は values.yaml のリソース制限を調整
```

---

## 8. アンインストール

### ArgoCDの完全削除

```bash
# Helmリリースを削除
helm uninstall argocd -n argocd

# Namespaceを削除（すべてのリソースが削除される）
kubectl delete namespace argocd

# CRDの削除（オプション - 他でArgoCDを使っていない場合のみ）
kubectl delete crd applications.argoproj.io
kubectl delete crd applicationsets.argoproj.io
kubectl delete crd appprojects.argoproj.io
```

```
⚠️ 注意
CRDを削除すると、すべてのArgoCD Application リソースが削除されます。
他のクラスタでArgoCDを使用している場合は、CRDは削除しないでください。
```

---

## クイックリファレンス

### よく使うコマンド一覧

```bash
# ===== インストール =====
kubectl create namespace argocd
helm repo add argo https://argoproj.github.io/argo-helm
helm repo update
helm install argocd argo/argo-cd -n argocd -f docs/argocd/values.yaml

# ===== 状態確認 =====
kubectl get pods -n argocd                    # Pod一覧
kubectl get svc -n argocd                     # Service一覧
argocd app list                               # アプリ一覧

# ===== 認証 =====
kubectl -n argocd get secret argocd-initial-admin-secret \
  -o jsonpath="{.data.password}" | base64 -d  # 初期パスワード取得
argocd login <IP>:30443 --insecure            # CLIログイン

# ===== アプリケーション管理 =====
argocd app create <NAME> --repo <URL> --path <PATH> \
  --dest-server https://kubernetes.default.svc --dest-namespace <NS>
argocd app sync <NAME>                        # 同期
argocd app delete <NAME>                      # 削除

# ===== トラブルシューティング =====
kubectl logs -n argocd -l app.kubernetes.io/part-of=argocd
kubectl describe pod -n argocd <POD>
```

---

## 参考リンク

- [ArgoCD 公式ドキュメント](https://argo-cd.readthedocs.io/)
- [ArgoCD GitHub](https://github.com/argoproj/argo-cd)
- [ArgoCD Helm Chart](https://github.com/argoproj/argo-helm/tree/main/charts/argo-cd)
- [GitOps とは](https://www.gitops.tech/)

---

```
📝 このドキュメントについて
作成日: 2026-01-18
対象: Raspberry Pi Kubernetes Cluster
ArgoCD バージョン: v2.13.x
Helm Chart バージョン: 最新安定版
```
